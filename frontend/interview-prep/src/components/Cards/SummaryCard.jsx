...
const SummaryCard = ({
    ...
    // console.log("topicsToFocus:", topicsToFocus);
    return (
        <div className='bg-white border border-gray-300/40 rounded-xl p-2 overflow-hidden cursor-pointer hover:shadow-xl shadow-gray-100 relative group'
            ...
            <div className='rounded-lg p-4 cursor-pointer relative' style={{
                            ...
                            <div>
                                ...
                                <p className='text-xs text-medium text-gray-900'>
                                    {Array.isArray(topicsToFocus)
                                        ? topicsToFocus.map((item) => item.session || item.note || "").join(", ")
                                        : typeof topicsToFocus === "object"
                                            ? JSON.stringify(topicsToFocus)
                                            : topicsToFocus}
                ...
                <button className='hidden group-hover:flex items-center gap-2 text-xs text-rose-500 font-medium bg-rose-50 px-6 py-1 rounded text-nowrap border border-rose-100 hover:border-rose-200 cursor-pointer absolute top-0 right-0'
